...
def train(hyperparams=None, data_path='data/emb', plot=True):
    ...
    output_file_path = "data/NCBI_gene_map.csv"
    ...
    save_file_= os.path.join(results_path, f'ppi_embeddings_head{num_heads}_dim{out_feats}_lay{num_layers}_epo{num_epochs}_initial.txt')
    ...
    save_path_pca = os.path.join(results_path, f'ppi_embeddings_pca_head{num_heads}_dim{out_feats}_lay{num_layers}_epo{num_epochs}.png')
    save_path_t_SNE = os.path.join(results_path, f'ppi_embeddings_t-SNE_head{num_heads}_dim{out_feats}_lay{num_layers}_epo{num_epochs}.png')
    save_path_heatmap_= os.path.join(results_path, f'ppi_embeddings_heatmap_stId_head{num_heads}_dim{out_feats}_lay{num_layers}_epo{num_epochs}.png')
    save_path_matrix = os.path.join(results_path, f'ppi_embeddings_matrix_stId_head{num_heads}_dim{out_feats}_lay{num_layers}_epo{num_epochs}.png')
    ...
    save_file = os.path.join(results_path, f'ppi_embeddings_head{num_heads}_dim{out_feats}_lay{num_layers}_epo{num_epochs}.txt')
...
def calculate_cluster_labels(net, dataloader, device, num_clusters=30):
    all_embeddings = []
    net.eval()
    with torch.no_grad():
